    ...
    <div class="w-100 h-100">
      ...
      >
        <div class="block pb-4 flex-1 md:flex-none md:pb-0">
          <app-list
            *ngFor="let grade of gradesNavigations; let i = index"
            [grade]="grade"
        ...
        <div class="flex-1 pt-4 px-2 md:pt-0 md:px-0 md:px-4 lg:px-0 lg:pl-8">
          ...
          <app-exercise-list
            (problemPick)="onProblemPick($event)"
          ></app-exercise-list>
...
export class ChapterComponent {
  ...
  get isQuizReady(): boolean {
    return isNotUndefined(this.qid)
  }
  ...
  onChapterPick(gradeIndex: number, chapterIndex: number) {
    for (
      let gradeCounter = 0;
      gradeCounter < this.gradesNavigations.length;
      gradeCounter++
    ) {
      ...
      for (
        ...
        chapters[chapterCounter].isActive =
          gradeIndex === gradeCounter && chapterIndex === chapterCounter
    }
  }

  onProblemPick(qid: string) {
    this.qid = 'GID11060001'
  }
