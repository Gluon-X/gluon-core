...
import { GradeNav } from '../containers/chapter.component'
  ...
  template: `
    <div class="block my-2">
      <div class="py-1 px-3 flex justify-between font-sans font-semibold rounded border-1 border-black cursor-pointer"
        ...
        <span class="truncate text-base">{{ grade?.name || '[undefined]' }}</span>
      ...
      <div class="transform transition-height duration-200"
           [ngClass]="{ 'h-0 overflow-hidden scale-0': !grade.isActive, 'scale-100': grade.isActive }">
...
export class ListComponent {
  ...
  handleItemPick(chapterIndex: number) {
    this.pick.emit(chapterIndex)
  }
...
@Component({
  ...
  template: `
    <div class="my-1 py-2 px-6 font-sans text-sm rounded cursor-pointer break-words"
